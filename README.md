...
- **三模块工作台** — 左侧一级导航切换 `星轨`、`每日记`、`笔记白板`，把记录、执行和空间整理放到同一个应用里
- **每日记** — 每天一页日记，支持 Markdown 编辑，提供详情视图、表格视图和带写作标记的月历跳转
- **星轨** — 围绕人生目标创建长期星轨，进入后优先展示今日该执行的动作；支持拆解终极目标、阶段成果和原子动作，并记录每次完成时间、自动汇总每日/每周完成度，也支持将原子动作设置为“每个工作日”
- **笔记白板总览** — 鸟瞰全部白板缩略图，点击即可进入具体白板内部继续编辑卡片
...
- **坚果云同步** — 配置坚果云 WebDAV 后，白板、每日记与星轨会共用一份云端快照；本地优先展示，保存后快速上传，后台定期检查同步；如果云端数据会覆盖大量本地内容，会暂停自动同步并要求你确认
...
- **每日记** — 进入后默认打开当天，支持详情视图和表格视图切换；右上角日历可以切换年月并跳到任意一天
- **星轨** — 先创建长期星轨，再把终极目标拆成阶段成果和原子动作；进入后会优先看到今天该执行的内容，每次完成都会留下时间记录，并自动判断每日/每周动作是否达标；如果某类动作只需要在上班日执行，也可以直接设置为“每个工作日”
- **笔记白板** — 默认先看到所有白板的鸟瞰图，点击缩略图即可进入具体白板内部继续整理卡片
...
- **右键菜单** — 在卡片上右键（macOS 双指点击），可进行最佳大小调整、拷贝链接、移动到其他画布、编辑、添加关联连线和删除
- **画布右键菜单** — 在画布空白处右键，优先创建卡片；白板里已有内容后，也可以继续补结构标注和分组区域
...
- **坚果云同步** — 填入坚果云 WebDAV 服务器地址、邮箱和应用密码，保存后自动开启同步。应用始终优先展示本地数据；保存后会快速上传本地改动，后台定期检查远端更新。如果云端数据会覆盖大量本地内容，会暂停自动同步并要求你确认后再继续。
- **星轨提醒** — 可开启桌面提醒、关闭窗口后后台运行、开机自启和免打扰时段，用来支撑星轨系统的持续提醒
