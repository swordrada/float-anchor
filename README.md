...
以画布为单位管理不同主题，左侧导航可创建、重命名和切换画布，白板总览则帮助你快速判断每张画布的内容规模。画布中可自由放置卡片、无边框文本、横幅标题与彩色分区；卡片支持拖拽、缩放、磁吸对齐、框选自动排布、移动到其他画布，以及用连线和可跳转链接把相关想法串起来。
...
画布可通过双指滑动或鼠标中键平移，通过捏合或 `Ctrl + 滚轮` 缩放；创建卡片、文本、标题和分区，以及自动排布、缩放和微移等高频操作均可使用快捷键完成。应用提供 Light / Dark 主题与设置内快捷键速查表，让长时间整理也保持舒适。

### 本地优先与可靠同步

所有数据默认保存在本地；如有跨设备需求，可选择坚果云 WebDAV 或 GitHub 仓库同步。保存后的变更会自动上传，并定期检查远端内容；当远端可能覆盖大量本地数据时，应用会暂停自动同步并要求确认。同步状态常驻侧栏，应用内更新支持检测、下载、暂停与继续。

## 下载安装

当前稳定版为 [`v1.0.19`](https://github.com/swordrada/float-anchor/releases/tag/v1.0.19)，可在 [Releases](https://github.com/swordrada/float-anchor/releases) 页面下载：

| 文件 | 平台 | 适用设备 |
|------|------|----------|
| `FloatAnchor-1.0.19-mac-arm64.dmg` | macOS | Apple Silicon (M1/M2/M3/M4) |
| `FloatAnchor-1.0.19-mac-x64.dmg` | macOS | Intel Mac |
| `FloatAnchor-1.0.19-win-setup.exe` | Windows | 64 位 Windows |
...
- **白板总览** — 点击左侧的 **白板总览**，按白板名称和内容规模快速找到要继续整理的内容
