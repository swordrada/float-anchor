...
- **右键菜单** — 在卡片上右键（macOS 双指点击），可进行最佳大小调整、拷贝链接、移动到其他画布、连接卡片、编辑和删除
- **画布右键菜单** — 在画布空白处右键，可创建空白卡片、创建标题、创建分区
...
> **注意**：Windows 下 Python 命令可能是 `python` 而非 `python3`，取决于安装方式。如果提示找不到命令，尝试换用另一个。
...
5. 点击该请求，在 **Headers**（标头）中找到 `Authorization` 字段，复制 `Bearer ` 后面的完整字符串
...
| 图片 | 通过 `--token` 参数下载到本地，支持 PNG / JPEG / GIF |
| 卡片布局 | 保留相对位置，自动紧凑排列 |
