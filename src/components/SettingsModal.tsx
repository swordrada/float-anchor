...
import { buildAppDataSnapshot, useStore } from '../store'
...
export default function SettingsModal() {
  ...
  const [activePanel, setActivePanel] = useState<'tracks' | 'sync' | 'backup' | 'update' | 'appearance' | 'danger'>('tracks')

  const CLEAR_CONFIRM_TEXT = '我确认只清空所有卡片与白板内容'
  ...
  const handleClearClick = useCallback(async () => {
    ...
    try {
      const res = await window.electronAPI.prepareClearAllCards()
      if (!res.success) {
        setClearStatus('no-backup')
        setClearMessage(res.error || '清空卡片前自动备份失败，请先导出备份后重试。')
        return
      }

      if (res.backupCreated) {
        const backupLabel = res.latestFileName ? `：${res.latestFileName}` : ''
        setClearMessage(`未检测到 1 天内的备份，已自动创建新备份${backupLabel}，请再次确认是否清空所有卡片。`)
      } else if (res.latestTimestamp) {
        setClearMessage(`已检测到 1 天内的最新备份（${formatBackupTime(res.latestTimestamp)}），可以继续清空所有卡片。`)
      } else {
        setClearMessage('已完成清空前备份检查，可以继续清空所有卡片。')
      }

      setClearStatus('ready')
    } catch {
      setClearStatus('no-backup')
      setClearMessage('清空卡片前自动备份失败，请先导出备份后重试。')
    }
  }, [])
  ...
  const handleClearConfirm = useCallback(async () => {
    ...
    setClearMessage('')
    try {
      const res = await window.electronAPI.clearAllCards()
      if (res.success) {
        setClearStatus('done')
        await useStore.getState().loadData()
        ...
        if (settings.webdav?.server) {
          try {
            await window.electronAPI.writeData(buildAppDataSnapshot(store))
            store.setSyncStatus('syncing')
            const syncRes = await window.electronAPI.webdavAutoSync(settings.webdav)
            if (!syncRes.success) {
              store.setSyncStatus('error')
              setClearMessage('所有卡片已清空，但坚果云同步失败，请稍后手动同步。')
            } else if (syncRes.action === 'needs-confirmation' && syncRes.decision) {
              store.setSyncDecision(syncRes.decision)
              store.setSyncStatus('warning')
              setClearMessage('所有卡片已清空，本地改动已准备同步到坚果云，请继续确认同步策略。')
            } else {
              ...
              if (syncRes.action === 'uploaded' || syncRes.action === 'downloaded') {
                ...
                setClearMessage('所有卡片已清空，并已同步到坚果云。')
              } else {
                store.setSyncStatus('idle')
                setClearMessage('所有卡片已清空，每日记与星轨数据已完整保留。')
              }
            }
          } catch {
            store.setSyncStatus('error')
  ...
  }, [clearInput, clearStatus, settings.webdav])
  ...
  const boardCount = useStore((s) => s.canvases.length)
  const cardCount = useStore((s) => s.canvases.reduce((sum, canvas) => sum + canvas.cards.length, 0))
  const journalEntryCount = useStore((s) => Object.keys(s.journal.entriesByDate).length)
  const starTrackCount = useStore((s) => s.trackSystem.tracks.length)
  const starTrackCheckinCount = useStore((s) => s.trackSystem.checkins.length)
  ...
  const handleResolveSync = useCallback(async (resolution: WebDAVSyncResolution) => {
    ...
    if (resolution === 'dismiss') {
      useStore.getState().setSyncDecision(null)
      useStore.getState().setSyncStatus('idle')
      return
    }
  ...
  const updateNotificationSettings = useCallback(async (
    updater: (value: typeof settings.notifications) => typeof settings.notifications,
  ) => {
    await saveSettings({
      ...settings,
        ...
        <div className="settings-modal-shell">
          <aside className="settings-sidebar">
            <div className="settings-sidebar-head">
              <h2>设置中心</h2>
              <p>把显示、星轨、同步和危险操作拆开，避免所有配置挤在一个小框里。</p>
            </div>

            <nav className="settings-sidebar-nav">
              <button className={activePanel === 'tracks' ? 'active' : ''} onClick={() => setActivePanel('tracks')}>
                <span>星轨</span>
                <small>提醒与后台节奏</small>
              </button>
              <button className={activePanel === 'sync' ? 'active' : ''} onClick={() => setActivePanel('sync')}>
                <span>坚果云同步</span>
                <small>云端缓存与冲突处理</small>
              </button>
              <button className={activePanel === 'backup' ? 'active' : ''} onClick={() => setActivePanel('backup')}>
                <span>备份与恢复</span>
                <small>本地压缩包备份</small>
              </button>
              <button className={activePanel === 'update' ? 'active' : ''} onClick={() => setActivePanel('update')}>
                <span>软件更新</span>
                <small>版本检查与安装</small>
              </button>
              <button className={activePanel === 'appearance' ? 'active' : ''} onClick={() => setActivePanel('appearance')}>
                <span>外观</span>
                <small>明暗主题</small>
              </button>
              <button className={activePanel === 'danger' ? 'active' : ''} onClick={() => setActivePanel('danger')}>
                <span>数据管理</span>
                <small>仅清空白板卡片</small>
              </button>
            </nav>

            <div className="settings-sidebar-snapshot">
              <span>当前快照范围</span>
              <strong>{boardCount} 个画布 / {cardCount} 张卡片</strong>
              <small>{journalEntryCount} 篇每日记 / {starTrackCount} 条星轨 / {starTrackCheckinCount} 条打卡</small>
            </div>
          ...
          <section className="settings-content">
            <div className="settings-content-topbar">
              <div>
                <h3>
                  {activePanel === 'tracks' ? '星轨提醒' :
                    activePanel === 'sync' ? '坚果云同步' :
                    activePanel === 'backup' ? '备份与恢复' :
                    activePanel === 'update' ? '软件更新' :
                    activePanel === 'appearance' ? '外观' : '数据管理'}
                </h3>
                <p>
                  {activePanel === 'tracks' ? '控制星轨提醒、后台常驻和免打扰节奏。' :
                    activePanel === 'sync' ? '白板、每日记和星轨会共用一份坚果云快照。' :
                    activePanel === 'backup' ? '导出与导入都覆盖白板、每日记、星轨和本地图片资源。' :
                    activePanel === 'update' ? '检查新版本并继续下载/安装流程。' :
                    activePanel === 'appearance' ? '设置当前工作区的显示风格。' :
                    '危险操作集中放在这里，避免误触。'}
                </p>
              </div>
            ...
            {activePanel === 'tracks' && (
              <div className="settings-panel-stack">
                <div className="settings-card">
                  <div className="settings-feature-banner">
                    <div>
                      <strong>星轨是新的长期目标系统</strong>
                      <p>它会持续跟踪阶段成果与原子动作，并在你偏离节奏时主动提醒。</p>
                    </div>
                    <div className="settings-feature-badge">NEW</div>
                  </div>
                </div>
                  ...
                  <div className="webdav-form">
                    <label className="settings-checkbox">
                      <input
                        ...
                        onChange={(event) => {
                          void updateNotificationSettings((value) => ({ ...value, enabled: event.target.checked }))
                        }}
                      />
                      <span>启用星轨桌面提醒</span>
                    </label>
                    <label className="settings-checkbox">
                      <input
                        type="checkbox"
                        checked={settings.notifications.runInBackground}
                        onChange={(event) => {
                          void updateNotificationSettings((value) => ({ ...value, runInBackground: event.target.checked }))
                        }}
                      />
                      <span>关闭窗口后继续在后台提醒</span>
                    </label>
                    <label className="settings-checkbox">
                      <input
                        type="checkbox"
                        checked={settings.notifications.launchAtLogin}
                        onChange={(event) => {
                          void updateNotificationSettings((value) => ({ ...value, launchAtLogin: event.target.checked }))
                        }}
                      />
                      <span>开机自动启动 FloatAnchor</span>
                    </label>
                    <label className="settings-checkbox">
                      <input
                        type="checkbox"
                        ...
                        onChange={(event) => {
                          void updateNotificationSettings((value) => ({
                            ...value,
                            quietHours: { ...value.quietHours, enabled: event.target.checked },
                          }))
                        }}
                      />
                      <span>启用免打扰时段</span>
                      ...
                      <label>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            void updateNotificationSettings((value) => ({
                              ...value,
                              quietHours: { ...value.quietHours, start: event.target.value },
                            }))
                          }}
                        />
                      </label>
                      <label>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            void updateNotificationSettings((value) => ({
                              ...value,
                              quietHours: { ...value.quietHours, end: event.target.value },
                            }))
                          }}
                        />
                      </label>
                    ...
                    <div className="settings-note">
                      如果你主动彻底退出应用，后台提醒会暂停；只是关闭窗口时，星轨仍会按设置继续提醒。
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activePanel === 'sync' && (
              <div className="settings-panel-stack">
                <div className="settings-card">
                  <div className="settings-coverage-grid">
                    <div>
                      <span>白板内容</span>
                      <strong>{boardCount} / {cardCount}</strong>
                      <small>画布与卡片会一并缓存到坚果云</small>
                    </div>
                    <div>
                      <span>每日记</span>
                      <strong>{journalEntryCount}</strong>
                      <small>日记正文与标题会进入同一份云端快照</small>
                    </div>
                    <div>
                      <span>星轨</span>
                      <strong>{starTrackCount}</strong>
                      <small>{starTrackCheckinCount} 条执行记录也会同步</small>
                    </div>
                  </div>
                </div>
                      ...
                      {connected && (
                        <>
                          <button onClick={handleManualSync} disabled={syncStatus === 'syncing' || !!syncDecision || !!syncResolveLoading}>
                            {syncStatus === 'syncing' ? '同步中...' : syncDecision ? '待确认' : '立即同步'}
                          </button>
                    ...
                    {syncDecision && (
                      <div className={`sync-decision-card ${syncDecision.risk === 'high' ? 'high-risk' : ''}`}>
                        ...
                        <div className="sync-decision-summary">
                          ...
                          <div className="sync-decision-row">
                            ...
                            <span>
                              {syncDecision.reason === 'remote-missing'
                                ? '无数据'
                                : formatSyncSummary(syncDecision.remoteSummary)}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="sync-decision-actions">
                          ...
                          >
                            {syncResolveLoading === 'keep-local' ? '上传中...' : '保留本地并同步云端'}
                          </button>
                          {syncDecision.reason !== 'remote-missing' && (
                            <button
                          ...
                          <button
                            onClick={() => handleResolveSync('dismiss')}
                            disabled={!!syncResolveLoading}
                          >
                            稍后再说
                          </button>
                ...
                <div className="settings-card">
                  <div className="settings-coverage-grid">
                    <div>
                      ...
                      <small>包含画布、标题、分区与连线</small>
                    </div>
                    <div>
                      <span>每日记</span>
                      <strong>{journalEntryCount}</strong>
                      <small>会跟随备份一起导出与恢复</small>
                    </div>
                    <div>
                      <span>星轨与打卡</span>
                      <strong>{starTrackCount}</strong>
                      <small>{starTrackCheckinCount} 条执行记录一并保存</small>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="data-management">
                    <div className="data-management-row">
                      <div className="data-management-item">
                        ...
                        <span className="data-hint">将白板、每日记、星轨和本地图片一起打包导出</span>
                      </div>
                      <div className="data-management-item">
                        <button
                        ...
                        <span className="data-hint">从备份文件恢复整份工作区快照</span>
                      </div>
                    </div>
                    {backupMessage && (
              ...
              <div className="settings-panel-stack">
                <div className="settings-card danger">
                  <div className="settings-danger-copy">
                    <strong>清空所有卡片</strong>
                    <p>这个操作只会清空白板中的画布、卡片、标题、分区和连线。每日记与星轨不会被删除。</p>
                  </div>
                  <div className="data-management-danger">
                    ...
                    <span className="data-hint danger-hint">此操作会先自动备份，再删除白板内容。</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="clear-confirm-overlay" onClick={(e) => { if (e.target === e.currentTarget) { setShowClearConfirm(false); setClearStatus('idle'); setClearMessage(''); setClearInput('') } }}>
            <div className="clear-confirm-modal">
              ...
              {clearStatus === 'preparing' ? (
                ...
                <>
                  <p className="clear-desc">此操作将永久删除所有画布、卡片、标题、分区和连线数据，但不会删除每日记与星轨。请输入以下文字确认：</p>
                  {clearMessage && (
                  ...
                  <div className="clear-confirm-actions">
                    ...
                    >
                      {clearStatus === 'clearing' ? '清空中...' : clearStatus === 'done' ? '已清空' : '确认清空卡片'}
                    </button>
                  </div>
                </>
              )}
